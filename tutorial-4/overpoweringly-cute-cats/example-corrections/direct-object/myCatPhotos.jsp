...
%>
<%
	AccessReferenceMap directObjectMap = (AccessReferenceMap) session.getAttribute("directObjectMap");
    if (directObjectMap == null) {
		 String basicObjects[] = {"public", "private", "delete"}; 
		 Set directObjectSet = new HashSet(Arrays.asList(basicObjects));
		 directObjectSet.add(request.getRemoteUser());
		 directObjectMap = new RandomAccessReferenceMap( directObjectSet );
		 session.setAttribute("directObjectMap", directObjectMap);
	}
%>
		...
		<div id="content">
			<div class="box" id="login">
				...
				<%
					...
					String userIndirectObj = request.getParameter("user");
					String actionIndirectObj = request.getParameter("action");
					String sourceIndirectObj = request.getParameter("source");
					Boolean deleteError = false;
					Class.forName("com.mysql.jdbc.Driver");
					Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/insecureCat", "catAdmin", "catPass");					
					if (actionIndirectObj == null || sourceIndirectObj == null || userIndirectObj == null) {
						deleteError = true;
					} else if (actionIndirectObj.isEmpty() || sourceIndirectObj.isEmpty() || userIndirectObj.isEmpty()) {
						deleteError = true;
					} else {
						String action =(String)directObjectMap.getDirectReference( actionIndirectObj );
						String source =(String)directObjectMap.getDirectReference( sourceIndirectObj );
						String user =(String)directObjectMap.getDirectReference( userIndirectObj );
					...
					if (rs1.next()) {
						...
						%>
							...
							<dl>
								<dt>Your private cute cat: </dt>
									...
									<dd><a href="<%= request.getContextPath() %>/protected/myCatPhotos.jsp?action=<%= directObjectMap.getIndirectReference("delete") %>&source=<%= directObjectMap.getIndirectReference("private") %>&user=<%= directObjectMap.getIndirectReference(request.getRemoteUser()) %>">Delete private cat image</a></dd>
							</dl>
						...
						%>		
							...
							<dl>
								...
								<dd><a href="<%= request.getContextPath() %>/protected/myCatPhotos.jsp?action=<%= directObjectMap.getIndirectReference("delete") %>&source=<%= directObjectMap.getIndirectReference("public") %>&user=<%= directObjectMap.getIndirectReference(request.getRemoteUser()) %>">Delete public cat image</a></dd>
							</dl>
						<%
						} else {
						%>
